        ...
        <div class="them-vao-gio-hang"><button title="Thêm sản phẩm này vào giỏ hàng của bạn"  value="' + i + '&0" style="button" class="them">Add <img class="icon-cart" src="image/icon/icon-cart.png"></button></div>
            ...
            <?php
               ...
               // echo '<script>alert("Khong tim thay ket qua!");</script>';
                 echo '<span class="so_trang block_trang">Khong tim thay ket qua!</span>';
            }
        }
...
<!-- phần tiêu đề của khung -->
    ...
    <?php 
        ...
        if ($current_page > 1 && $total_page > 1){
            echo '<a class="so_trang" href="index.php?search=no&submit=TÌM&page_trang='.($current_page-1).'">Prev</a>  ';
        }
        ...
        for ($i = 1; $i <= $total_page; $i++){
            ...
            if ($i == $current_page){
                ...
                echo '<a class="so_trang" href="index.php?search=no&submit=TÌM&page_trang='.$i.'">'.$i.'</a>  ';
            }
        }

        // nếu current_page < $total_page và total_page > 1 mới hiển thị nút prev
        if ($current_page < $total_page && $total_page > 1){
            echo '<a class="so_trang" href="index.php?search=no&submit=TÌM&page_trang='.($current_page+1).'">Next</a>  ';
        }
       ?>
    </div>
    <!-- <div class="chuyen_trang clear_both">
